...
- (SCFilter *)createAnimatedFilter {
    ...
    Float64 assetDuration = CMTimeGetSeconds(_recordSessions.assetRepresentingSegments.duration);
...
- (void)backToEvent {
    [self performSegueWithIdentifier:@"toEvent" sender:nil];
}
...
- (void)viewDidLoad
{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Back" style:UIBarButtonItemStylePlain target:self action:@selector(backToEvent)];
...
- (void)viewWillAppear:(BOOL)animated {
    ...
    [_player setItemByAsset:_recordSessions.assetRepresentingSegments];
	[_player play];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.destinationViewController isKindOfClass:[SCEditVideoViewController class]]) {
        ...
        editVideo.recordSession = self.recordSessions;
    }
}
...
- (void)saveToCameraRoll {
    ...
    SCAssetExportSession *exportSession = [[SCAssetExportSession alloc] initWithAsset:self.recordSessions.assetRepresentingSegments];
    exportSession.videoConfiguration.filter = currentFilter;
    exportSession.videoConfiguration.preset = SCPresetHighestQuality;
    exportSession.audioConfiguration.preset = SCPresetHighestQuality;
    exportSession.videoConfiguration.maxFrameRate = 35;
    exportSession.outputUrl = self.recordSessions.outputUrl;
