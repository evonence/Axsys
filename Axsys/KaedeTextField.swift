 ...
 */
public class KaedeTextField: TextFieldEffects {
     ...
     */
    dynamic public var placeholderColor: UIColor? {
        didSet {
            updatePlaceholder()
        }
    }
     ...
     */
    dynamic public var placeholderFontScale: CGFloat = 0.8 {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    override public func animateViewsForTextEntry() {
        UIView.animateWithDuration(0.35, delay: 0.0, usingSpringWithDamping: 0.8, initialSpringVelocity: 2.0, options: .BeginFromCurrentState, animations: ({
            self.placeholderLabel.frame.origin = CGPoint(x: self.frame.size.width * 0.6, y: self.placeholderInsets.y)
        }), completion: nil)
